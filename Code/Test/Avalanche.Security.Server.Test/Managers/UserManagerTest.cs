...
#pragma warning disable CA1707 // Identifiers should not contain underscores
    public class UsersManagerTest
    {
        ...
        [Fact]
        public async Task UserManager_VerifyAdminUserLogin_LoginFails_WrongPassword()
        {
            // Arrange
            const string providedPassword = "UserProvidedPassword";
            var user = Fakers.GetUserFaker().Generate();
            ...
            // Act
            var (loginValid, userResult) = await usersManager.VerifyAdminUserLogin(user.UserName, providedPassword).ConfigureAwait(false);
        ...
        [Fact]
        public async Task UserManager_VerifyAdminUserLogin_LoginFails_NoUser()
        {
            // Arrange
            const string providedPassword = "UserProvidedPassword";
            var user = Fakers.GetUserFaker().Generate();
        ...
        [Fact]
        public async Task UserManager_VerifyAdminUserLogin_LoginFails_UserIsNotAdmin()
        {
            // Arrange
            const string providedPassword = "UserProvidedPassword";
            var user = Fakers.GetUserFaker().Generate();
            ...
            // Assert
            hashMoq.Verify(x => x.PasswordMatches(It.IsAny<string>(), It.IsAny<string>()), Times.Once());
            Assert.False(loginValid);
            Assert.NotNull(userResult);
            Assert.Equal(user, userResult);
        }
        ...
        [Fact]
        public async Task UserManager_VerifyAdminUserLogin_LoginSucceeds_UserIsAdmin()
        {
            // Arrange
            const string providedPassword = "UserProvidedPassword";
            var user = Fakers.GetUserFaker().Generate();
